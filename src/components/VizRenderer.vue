<template>
  <div class="renderer__wrapper" ref="rendererWrapper">
    <div class="vizhtml">
      ...
      <router-view />
      <!-- here goes VitOverview, VizTag, VizDetail via router -->
      <VizTransition />
      <VizInput />
    </div>
  </div>
...
export default {
  ...
  watch: {
    inverted: function(newValue) {
      ...
      TweenLite.to(this.PIXIApp.stage.filters[0], durations.invert, {
        alpha: targetAlpha,
  ...
  methods: {
    ...
    updateDebugGrid() {
      ...
      let sprite = this.debugContainer.addChild(
      ...
      sprite = this.debugContainer.addChild(
        new PIXI.Sprite(PIXI.Texture.WHITE)
      );
      sprite.tint = 0x0000ff;
      sprite.height = 12;
  ...
  mounted: function() {
    ...
    this.PIXIApp.view.oncontextmenu = function() {
      return false;
    };
    const touchListener = () => {
      console.log("we are on a touchdevice");
      this.$store.state.isTouchDevice = true;
